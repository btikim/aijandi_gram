...
class RPLidar(object):    # object 라는 클래스를 상속 받아서 사용한다.   RPLidar라는 클래스를 만들었다.
    '''Class for communicating with RPLidar rangefinder scanners'''
        
    def __init__(self, port, baudrate=115200, timeout=1, logger=None):   # 초기 생성자는 이렇게 선언한다.
        ...
        '''
        self._serial = None             # 변수 선언할 때 self. 을 반드시 붙여줘야 한다.
        self.port = port
    ...
    def connect(self):              
        ...
        if self._serial is not None:        # 클래스 안에서 함수를 만들 때는 self를 써서 변수를 사용해야 한다.
            self.disconnect()
        try:
